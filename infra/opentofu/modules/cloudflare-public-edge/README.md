...
records, Full (strict), Always Online disabled, zone-level authenticated origin
pulls, cache bypass, representation-preserving settings, and the reserved
`/cdn-cgi/` block.
...
it is already active. Cloudflare's public leaf and metadata may therefore enter
encrypted state; its private key never enters configuration, plans, or state.
