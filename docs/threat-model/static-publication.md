...
| Concurrent or replayed jobs | Let only the authenticated issuer allocate a root-owned job and correlation. Bind operator, operation, target, canonical request, artifact, and expected state; durably claim before execution; retain the exact source and candidate manifests in lifecycle authority; reject candidates that differ outside the operation-specific transformation or disagree with observed recovery state; serialize publication; and make an exact retry return one immutable result. Unknown IDs and changed bindings allocate nothing. |
