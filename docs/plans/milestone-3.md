...
Implementation status (2026-08-30): in progress. The first review boundary
installs the dedicated, password-disabled SSH identity, root-owned key binding
and principal, forced-command denial adapter, and account-specific OpenSSH
restrictions. Its installed-host acceptance suite uses a real SSH daemon to
prove public-key entry reaches only the disabled adapter while shell commands,
PTY, forwarding, environment injection, SFTP, and SCP remain unavailable and
the static state tree remains unchanged. M3.2 supplied the strict request, job,
result, and identifier contracts; M3.3 supplied immutable job and correlation
admission, capacity and rate limits, audit, and restart-safe state; M3.4
supplied bounded artifact handling; and the completed M3.5 live gate supplied
the installed, root-owned dark-host layout, disabled issuance gate, inert
worker boundary, and pinned host-agent selector. No architecture or product
decision blocks the remaining hermetic M3.6 implementation.
