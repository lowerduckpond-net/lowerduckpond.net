...
- Status: proposed; implementation starts only after this plan and its ADR merge
...
configuration, Caddy environment files, archive/backup credential files and
generated secret-bearing host inputs. Tenant archive bundles stay exclusively
...
Persist a root-generated audit lineage UUIDv7, initialized only after validating
the entire pre-migration local chain. Its root record binds the namespace digest
...
Build witnesses from the restore-verified segment, bind their exact digest in
its descriptor, and compare regenerated bytes before index commit. All ordinary
...
Before restoring bytes, stop/mask Caddy, operator ingress, lifecycle workers,
...
unmask/start Caddy through the normal invocation-fenced verifier. Keep operator
ingress and mutation/maintenance timers stopped until the selected runtime and
routes match the verified state. A failed start retains the restore marker and
ordinary bounded attempt evidence; interruption after unmask cannot bypass the
pre-start restore gate. Reboot and exact old-result replay are final acceptance
steps. Mark `complete` durably before restoring schedules/ingress idempotently.
...
Install in this order: readers/schema support and retention guard; coherent
backup descriptor; audit lineage/index migration; rotation machinery (off);
...
| P2: coherent backup and recovery descriptor | P1. Complete source/writer inventory, exact capture authority, bounded units and `backup-coherence` coverage. No rotation. |
| P3: protected audit verification and maintenance | P2. Schemas/index readers/witnesses, lineage migration, orphan discovery, retention guard, health and `audit-protection`. Rotation remains off. |
...
| P5: restored-host reconstruction | P4. Gated bootstrap, exact-version and timeline checks, missing-input decisions, generation mappings, independently runnable positive/negative recovery groups and operator restore instructions. |
