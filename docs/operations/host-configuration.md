...
require active Caddy. Their local HTTPS probe requires either the fixture
...
that provenance before dispatch, and creates a new deployment through the
ordinary prepare, publish, activate, and recovery transaction. Only the content
...
Import accepts active, suspended, or archived source manifests and consumes
intake only after the terminal result is validated.
