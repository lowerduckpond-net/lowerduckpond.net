...
The edge, Caddy, and audit credentials use account-token verification endpoints;
...
help diagnose a retained fixture; they do not authorize a new upload or cleanup,
and a subsequent successful worker exit can represent a validated rollback.
...
for each zone, independently of the Rulesets inventory. The separate user token
must prove empty
[legacy Page Rules](https://developers.cloudflare.com/api/resources/page_rules/methods/list/)
in both zones using only Page Rules Read. An unavailable or nonempty inventory fails the check before
...
Cloudflare request failures report a sanitized endpoint template and HTTP status,
or a network/TLS error category. Return that line for diagnosis; it excludes
credential values, raw response bodies, queries, and account/zone/token IDs.
